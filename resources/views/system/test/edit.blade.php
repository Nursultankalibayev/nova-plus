...
@section('content')
    <div class="row bg-title">
        <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
            <h4 class="page-title">Анализы | Изменение</h4> </div>
                ...
                <div class="panel-wrapper p-b-10 collapse in">
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="white-box">
                                @if(count($rows['test']))
                                    <form action="{{route('test.update',$rows['test']->id)}}" method="POST" class="form-horizontal" enctype="multipart/form-data">
                                        {{csrf_field()}}
                                        {{method_field('PUT')}}
                                        <div class="form-group">
                                            <label class="col-md-12">Заголовок RU *</label>
                                            <div class="col-md-12">
                                                <input type="text" name="title_ru" class="form-control" value="{{$rows['test']->title_ru}}">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label class="col-md-12">Заголовок KK</label>
                                            <div class="col-md-12">
                                                <input type="text" name="title_kk" class="form-control" value="{{$rows['test']->title_kk}}">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label class="col-md-12">Заголовок EN</label>
                                            <div class="col-md-12">
                                                <input type="text" name="title_en" class="form-control" value="{{$rows['test']->title_en}}">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                                ...
                                                <select id="category_id" class="form-control" name="category_id">
                                                    @if(count($rows['categories'] ))
                                                        @foreach($rows['categories'] as $item)
                                                            <option {{$rows['test']->category_id == $item->id ? 'selected' : ''}} value="{{$item->id}}">{{$item->title_ru}}</option>
                                                        @endforeach
                                                    @endif
                                                </select>
                                            </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-12">
                                                <input type="text" name="price" class="form-control" value="{{$rows['test']->price}}">
                                            </div>
                                        </div>
